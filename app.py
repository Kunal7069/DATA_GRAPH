...
@app.route('/api/create_nodes', methods=['POST'])
def create_node():
    data = request.json
...
@app.route('/api/create_edges', methods=['POST'])
def create_edge():
    data = request.json
...
@app.route('/api/get_edges', methods=['GET'])
def get_edges():
...
def home():
    return jsonify({"message": "SERVER IS RUNNING"}), 201
